    ...
    <AdminLayout>
        ...
        <template #header-title>
            <div class="flex items-center gap-2">
                ...
                <span class="rounded-md border border-slate-700 bg-slate-800 px-2 py-0.5 font-mono text-[10px] text-slate-400">
                    Stage 1 Core
                </span>
            </div>
        </template>
        ...
        <!-- Quick Action Bar -->
        <div class="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-800 bg-slate-900/60 p-4">
            <div>
                <h2 class="text-xs font-bold text-slate-200">
                    Pusat Pengendalian Ekosistem AksaraEdu
                </h2>
                <p class="text-[11px] text-slate-400">
                    Kelola lisensi kriptografis RSA-4096, registrasi sekolah mitra, dan distribusi paket rilis.
                </p>
            </div>
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <Card class="border-slate-800 bg-slate-900 p-5">
                <div class="flex items-center justify-between">
                    ...
                    <div class="rounded-lg border border-emerald-500/20 bg-emerald-500/10 p-2 text-emerald-400">
                        <School class="h-4 w-4" />
                    </div>
                </div>
                ...
                <div class="mt-1 flex items-center gap-2 text-[11px] text-slate-400">
                    <span class="font-semibold text-emerald-400">{{ stats.total_beli_putus }} Beli Putus</span>
                    <span>•</span>
                    <span class="font-semibold text-teal-400">{{ stats.total_langganan }} SaaS Cloud</span>
                </div>
            </Card>
            ...
            <Card class="border-slate-800 bg-slate-900 p-5">
                <div class="flex items-center justify-between">
                    ...
                    <div class="rounded-lg border border-teal-500/20 bg-teal-500/10 p-2 text-teal-400">
                        <Coins class="h-4 w-4" />
                    </div>
                </div>
                <p class="mt-2 text-2xl font-extrabold text-white">
                ...
                <p class="mt-1 flex items-center gap-1 text-[11px] text-emerald-400">
            ...
            <Card class="border-slate-800 bg-slate-900 p-5">
                <div class="flex items-center justify-between">
                    <span class="text-xs font-semibold text-slate-400">Pusat Rilis & Patch</span>
                    <div class="rounded-lg border border-indigo-500/20 bg-indigo-500/10 p-2 text-indigo-400">
            ...
            <Card class="border-slate-800 bg-slate-900 p-5">
                <div class="flex items-center justify-between">
                    <span class="text-xs font-semibold text-slate-400">Status Otoritas Lisensi</span>
                    <div class="rounded-lg border border-emerald-500/20 bg-emerald-500/10 p-2 text-emerald-400">
        ...
        >
            <div class="mb-3 flex items-center justify-between">
                <div class="flex items-center gap-2">
                    ...
                    <h3 class="text-xs font-bold tracking-wider text-amber-300 uppercase">
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-12">
            ...
            <div class="lg:col-span-7">
                <Card class="flex h-full flex-col border-slate-800 bg-slate-900 p-6">
                    <div class="flex items-center justify-between border-b border-slate-800 pb-4">
                        <div class="flex items-center gap-2">
                            <Activity class="h-4 w-4 animate-pulse text-emerald-400" />
                    ...
                    <div class="mt-2 flex-1 divide-y divide-slate-800/80 overflow-y-auto">
                        <div
                        ...
                        >
                            <div class="space-y-0.5">
                                <p class="font-semibold text-slate-200">
                                    {{ log.lisensi?.klien_sekolah?.nama_sekolah || 'Sekolah Mitra' }}
                                </p>
                                <div class="flex items-center gap-2 text-[11px] text-slate-400">
                                    <span>{{ log.domain_terdeteksi }}</span>
                                    <span>•</span>
                                    <span class="font-mono text-emerald-400">v{{ log.versi_lms }}</span>
                                </div>
                            </div>
                            <div class="text-right">
                                ...
                                <p class="text-[10px] text-slate-500">
                                    {{ new Date(log.waktu_ping).toLocaleTimeString('id-ID') }}
                                </p>
                            </div>
                        </div>
                        <div
            ...
            <div class="space-y-6 lg:col-span-5">
                ...
                <Card class="border-slate-800 bg-slate-900 p-6">
                    ...
                    <div class="space-y-2.5 text-xs text-slate-400">
                        <div class="flex items-start gap-2.5">
                            <span class="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-slate-800 text-[10px] font-bold text-emerald-400">1</span>
                            <p>Daftarkan sekolah di menu <strong>Sekolah</strong> dengan NPSN valid.</p>
                        </div>
                        <div class="flex items-start gap-2.5">
                            <span class="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-slate-800 text-[10px] font-bold text-emerald-400">2</span>
                            <p>Lisensi otomatis terbit dan berkas <code>aksaraedu.lic</code> atau bundle <code>.zip</code> langsung dapat diunduh.</p>
                        </div>
                        <div class="flex items-start gap-2.5">
                            <span class="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-slate-800 text-[10px] font-bold text-emerald-400">3</span>
                            <p>Serahkan berkas lisensi atau paket siap pasang ke admin sekolah untuk mode Beli Putus / SaaS.</p>
                        </div>
                    </div>
                </Card>
                ...
                <Card class="border-slate-800 bg-slate-900 p-6">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        ...
                        >
                            <span class="text-[11px] text-slate-400">Tiket Terbuka</span>
                            <p class="text-lg font-bold text-white">{{ stats.total_tiket_open }}</p>
                        </Link>
                        <Link
                            href="/admin/leads"
                            class="rounded-xl border border-slate-800 bg-slate-800/50 p-3 transition-colors hover:bg-slate-800"
                        >
                            <span class="text-[11px] text-slate-400">Leads Baru</span>
                            <p class="text-lg font-bold text-white">{{ stats.total_leads_baru }}</p>
                        </Link>
                    </div>
                </Card>
            </div>
