    ...
    <AdminLayout>
        ...
        <template #header-title>
            <h1 class="text-base font-bold tracking-tight text-slate-100">
                Tiket Bantuan & Pelacakan SLA Garansi
            </h1>
        </template>
        ...
        <div class="space-y-6">
            ...
            >
                ...
                >
                    <Plus class="mr-1.5 h-4 w-4" /> Buka Tiket Baru
                </Button>
            </div>
            ...
            <div class="grid grid-cols-1 gap-4">
                <Card
                    ...
                    :key="tiket.id"
                    class="border-slate-800 bg-slate-900 p-5 transition-colors hover:border-slate-700"
                >
                    <div
                    ...
                    >
                        <div class="flex items-center gap-3">
                            <span
                                class="rounded border border-slate-700 bg-slate-800 px-2.5 py-1 font-mono text-xs font-bold text-white"
                            >
                                {{ tiket.nomor_tiket }}
                            </span>
                            <div>
                                ...
                                >
                                    {{ tiket.klien_sekolah?.nama_sekolah }}
                                </p>
                            </div>
                        </div>
                    ...
                    <div
                        v-if="tiket.tanggapan_admin"
                        class="mb-3 rounded-xl border border-slate-800 bg-slate-950/70 p-3 text-xs text-slate-300"
                    >
                        <p
                            class="mb-0.5 flex items-center gap-1 text-[11px] font-bold text-emerald-400"
                        >
                            <MessageSquare class="h-3 w-3" /> Tanggapan Tim
                            Support:
                        </p>
                    ...
                    <!-- Footer Actions -->
                    <div
                        class="flex items-center justify-between border-t border-slate-800/80 pt-2 text-[11px] text-slate-400"
                    >
        ...
        <Modal
            ...
            title="Buka Tiket Bantuan Satuan Pendidikan"
            maxWidth="lg"
        >
            <form @submit.prevent="submitCreateTicket" class="space-y-4">
                ...
                <div>
                    <label
                        class="mb-1 block text-xs font-semibold text-slate-300"
                        >Judul Kendala / Masalah</label
                    >
                    <Input
                        v-model="createForm.judul_masalah"
                        placeholder="Contoh: Kesalahan submit jawaban CBT rombel XII RPL"
                        required
                    />
                </div>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <select
                            ...
                            class="w-full rounded-lg border border-slate-700 bg-slate-800 px-3 py-2 text-xs text-slate-100"
                        >
                            <option value="bug_sistem">
                                Bug Sistem / Software
                            </option>
                            <option value="pertanyaan_fitur">
                                Pertanyaan Modul & Fitur
                            </option>
                            <option value="instalasi">
                                Instalasi & Konfigurasi Server
                            </option>
                            <option value="darurat">
                                Darurat (Ujian Sedang Berlangsung)
                            </option>
                        </select>
                    </div>
                    <div>
                ...
                <div class="flex items-center gap-2 text-xs text-slate-300">
                    <input
                        type="checkbox"
                        v-model="createForm.is_garansi_claim"
                        id="garansi_check"
                        class="rounded border-slate-700 bg-slate-800 text-emerald-500"
                    />
                    <label for="garansi_check" class="cursor-pointer"
                        >Klaim Garansi Bugfix Resmi (Prioritas SLA 24
                        Jam)</label
                    >
                </div>
        ...
        <Modal
            ...
            title="Tanggapi & Update Status Tiket"
            maxWidth="md"
        >
            <form @submit.prevent="submitResponse" class="space-y-4">
                ...
                <div>
                    <label
                        class="mb-1 block text-xs font-semibold text-slate-300"
                        >Catatan / Tanggapan Tim Support</label
                    >
                    <textarea
                ...
                >
                    <Button
                        @click="isResponseModalOpen = false"
                        variant="ghost"
                        size="sm"
                        >Batal</Button
                    >
